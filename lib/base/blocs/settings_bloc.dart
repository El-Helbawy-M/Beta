...
class SettingsBloc extends Cubit<AppStates> {
  ...
  static SettingsBloc get instance =>
      BlocProvider.of(CustomNavigator.navigatorState.currentContext!);
  init() {
    emit(Loading());
    // init theme
    String theme = SharedHandler.instance!
        .getData(key: SharedKeys().theme, valueType: ValueType.string);
    if (theme.isEmpty) {
    ...
    // init lang
    String lang = SharedHandler.instance!
        .getData(key: SharedKeys().lang, valueType: ValueType.string);
    settingsModel.lang = (lang != "") ? lang : "en";
    emit(Done());
  }

  SettingsModel settingsModel =
      SettingsModel(lang: "en", theme: ColorsThemeType.lightTheme);

  set updateLang(String value) {
    if (value != currentLang())
      SharedHandler.instance!.setData(SharedKeys().lang, value: value);
  ...
  set updateTheme(ColorsThemeType value) {
    SharedHandler.instance!
        .setData(SharedKeys().theme, value: value.toString());
